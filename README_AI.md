...
The dashboard's evaluation window is **2023Q1 → latest BCA quarter**.
Both panels of the Wedge Decomposition section (the f-stat bar chart
and the cf-time-series line chart) are anchored at the same
`eval_window_start` so they describe the same window. The hypothesis
prompt is fed:
- f-statistics per wedge over the window
- Counterfactual peak swings (signed deviation from window-start
  baseline) — captures the offsetting-wedge story when observed
  output is near-flat and the inverse-SSR f-stat would otherwise
  degenerately favor the inertest cf
- Observed data range across the window
- An explicit methodological caveat instructing the LLM to lead with
  offsetting-swings narrative when |cf swing| of multiple wedges is
  large relative to the data range

### Step 6b — Events Log (`scripts/generate_events.py`)

Per-quarter US macro event log generated by Gemini 2.5 Flash with
**Google Search grounding** (`tools=[GoogleSearch()]`). Idempotent:
scans `data/events.md` for existing `## YYYYQX` headers and skips
quarters already present unless `--force`. Each quarter is generated
once and frozen. Output is structured into five fixed buckets
(Monetary / Fiscal / Regulatory / Tech / Geopolitical) with at most
3 dated, factual entries per bucket. Designed to be ingested by a
future revision of the hypothesis prompt so candidate mechanisms
anchor to dated events rather than the LLM's narrative-of-the-moment
training prior.
